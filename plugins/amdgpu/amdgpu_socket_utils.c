...
int send_parallel_restore_cmd(parallel_restore_cmd *restore_cmd)
{
	...
	pr_debug("Parallel restore: begin to send cmd_head\n");
	ret = send_metadata(sock_fd, restore_cmd);
	if (ret) {
		goto err;
	}

	pr_debug("Parallel restore: send gpu_ids\n");
	ret = send_gpu_ids(sock_fd, restore_cmd);
	if (ret) {
		goto err;
	}

	pr_debug("Parallel restore: begin to send entries\n");
	ret = send_cmds(sock_fd, restore_cmd);
	if (ret) {
		goto err;
	}

	pr_debug("Parallel restore: begin to send fds\n");
	ret = send_dmabuf_fds(sock_fd, restore_cmd);

	pr_debug("Parallel restore: connected over\n");
...
int recv_parallel_restore_cmd(parallel_restore_cmd *restore_cmd)
{
	...
	pr_debug("Parallel restore: begin to recv cmd_head\n");
	...
	pr_debug("Parallel restore: begin to recv entries\n");
	ret = recv_cmds(client_fd, restore_cmd);
	if (ret) {
		goto err;
	}

	pr_debug("Parallel restore: begin to recv fds\n");
	ret = recv_dmabuf_fds(client_fd, restore_cmd);

	pr_debug("Parallel restore: recv over\n");
